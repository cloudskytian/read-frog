...
const DEPRECATED_LLM_PROVIDER_MODEL_ALIASES: Record<string, Record<string, string>> = {
  anthropic: {
...
export function normalizeDeprecatedLLMProviderModel(provider: string, model: string): string {
  return DEPRECATED_LLM_PROVIDER_MODEL_ALIASES[provider]?.[model] ?? model
}

export function getDeprecatedLLMProviderModelNormalizationResult<T>(config: T): { config: T, changed: boolean } {
  if (!config || typeof config !== "object") {
    return { config, changed: false }
  }

  const value = config as T & { providersConfig?: unknown }
  if (!Array.isArray(value.providersConfig)) {
    return { config, changed: false }
  }

  let changed = false
  const nextProvidersConfig = value.providersConfig.map((providerConfig) => {
    if (!providerConfig || typeof providerConfig !== "object") {
      return providerConfig
    }

    const providerValue = providerConfig as {
      provider?: unknown
      model?: { model?: unknown }
    }

    if (typeof providerValue.provider !== "string" || !providerValue.model || typeof providerValue.model.model !== "string") {
      return providerConfig
    }

    const nextModel = normalizeDeprecatedLLMProviderModel(providerValue.provider, providerValue.model.model)
    if (nextModel === providerValue.model.model) {
      return providerConfig
    }
...
export function normalizeDeprecatedLLMProviderModelsInConfig<T>(config: T): T {
  return getDeprecatedLLMProviderModelNormalizationResult(config).config
}
